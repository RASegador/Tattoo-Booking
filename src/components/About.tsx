    ...
    <section id="about" className="relative py-32 px-6">
      <div className="mx-auto max-w-7xl grid md:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <div className="card-hover-red absolute -bottom-8 -right-8 glass-panel rounded-xl p-6 hidden sm:block border border-gold/25">
        ...
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp}>
          ...
          <div className="flex flex-wrap gap-3">
            {specialties.map((s) => (
              <span
                key={s}
                className="px-4 py-2 text-xs tracking-[0.15em] uppercase border border-white/15 text-white/70 rounded-full hover:border-gold-light hover:text-gold-light transition-colors"
              >
                {s}
              </span>
            ))}
      ...
      >
        ...
        <TribalPatternIcon className="w-40 h-4 text-gold/40 mx-auto mt-8" aria-hidden />
      </motion.div>
    </section>
  );
}
